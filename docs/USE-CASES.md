...
**Section A and the C-rows re-read against `v0.7.7`, 2026-09-03.** Rows A3, C9
and the new C13 were rewritten; A3 had drifted in three separate ways and its
old text is kept as A3′ rather than deleted. Two of those three were caught by
re-reading the code, and **the third was caused by shipping `v0.7.7` and not
updating the row** — this file's own rule, broken in the same week it was being
quoted at other people. Sections B, D and E have **not** been re-read since
`v0.6.3` and should be treated with the same suspicion.
...
| A3 | Parked, no position at all — no fix now, nothing usable from the drive | Ask, say plainly that the location is unknown, and **offer a way to supply it**. The use case publishes no verdict about the spot, only that it has none | A failed fix means "we don't know where you are", not "the car is nowhere" | **BUILT** | Rewritten against `v0.7.7`; the previous note was stale in three ways and is kept in the row below rather than deleted. Today: the map says *"Parked — but the location is unknown"* and offers **"Where is the car?"**, which enters pin placement without needing a pin; placing sets `lastParkLocation`, `detectedParkLocation` (the first placement becomes the cap's anchor), `parkedOutside`, `lastZoneCode` and `parkedOutsideKnown = true`. `parkedOutside` is **not** overwritten by this branch, so the wire keeps the last published value — but `parkedOutsideKnown = false` now rides with it, and `ClaimGuard` treats unknown as no more permissive than known |
| A3′ | *(what the row said before, kept because being wrong twice is the thing this file exists to prevent)* | — | — | **RETRACTED 2026-09-03** | Three claims, all false by the time anyone read them. **"The old pin is not kept — it was cleared when the car connected (`v0.3.4`)"**: false since `v0.6.8`, which deliberately stopped clearing it — *"deleting it made that the only copy, so a park that then failed to produce a position left no car location at all"*. **"`parkedOutsideKnown` … nothing reads it"**: false since `v0.6.5` — `SyncStateWorker` publishes it (`SharedSync.kt:161`) and `ClaimGuard.kt:46` reads it. **And the row implied being asked was the end of it**, which was true and is the bug `v0.7.7` fixed. Two of the three were found by an agent re-reading the code rather than the document; the third was introduced by shipping a fix and not updating the row, which is the rule in `CLAUDE.md` being broken by the person who quotes it |
...
| C9 | The pin is corrected and paid/free does not change | Move the pin, re-sync, ask nothing | Nothing about the obligation changed, so there is nothing to put to the user | **BUILT** | Updated `v0.7.7`: confirming any correction now also sets `parkedOutsideKnown = true`, including on this silent path. It did not, and the omission was invisible here — detection had already set the flag on a normal park — but fatal on A3's, where placing the car by hand still left the other phone blocked by it (`ClaimGuard.kt:46` needs both `parkedOutside` and `parkedOutsideKnown`) |
| C13 | The pin is placed for a park that **never had a position** (A3) | Resolve the zone and always put the claim or hand-back question, whichever applies | This is not a correction — it is the first thing ever said about that spot, so there is no previous answer for it to be "unchanged" from | **BUILT** `v0.7.7` | The distinction is load-bearing rather than pedantic. `CarBluetoothReceiver` writes `parkedOutside = false` at the start of every drive, so comparing against it made a first placement in a free zone compute "no change" and, per C9, ask nothing — leaving the spot unknown for ever and the other car blocked. `correctionFor` now takes a **nullable** `wasParkedOutside`; null yields `Flip.UNKNOWN_BEFORE`, which always states what it resolved and always routes to the question. The 300 m cap is postponed by exactly one placement, never removed: with no anchor the first placement is uncapped and then becomes the anchor |
